...
现在，看看你的周围——你看到的到处都是物体：书籍、建筑物、食物，甚至你自己。这些物体有两个主要组成部分：1) 一些相关属性（例如重量、颜色、大小、坚固性、形状等）；2) 它们可以表现出一些行为（例如被打开、使其他东西变热等）。这些属性和行为是不可分割的。
...
所有（非静态）成员函数都必须使用该类类型的对象来调用。在本例中，是被调用`today`的对象。`print()`
...
在成员示例中，我们以`year`、`month`、`day`,和 的形式访问成员。
...
当`kisses()`成员函数执行时，标识符`name`不使用成员选择运算符 (.)，因此它引用隐式对象，即`joe`。因此，这将解析为`joe.name`。`person.name`使用成员选择运算符，因此它不引用隐式对象。由于`person`是 的引用`kate`，因此这将解析为`kate.name`。
